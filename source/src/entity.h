...
enum { ENT_PLAYER = 0, ENT_BOT, ENT_CAMERA, ENT_BOUNCE, ENT_VEHICLE };
...
//START THE BRAND NEW CODE!!!

class vehicleent : public dynent
{
private:
    int health;
    float armor;
    float speed;
    float acceleration;
    float maxroll; // Maximum roll angle due to movement or damage
...
public:
    ...
    vehicleent() : health(100), armor(50.0f), speed(0.0f), acceleration(5.0f), maxroll(10.0f), team(0), damage(0), destroyed(false)
    {
        type = ENT_VEHICLE;
        name[0] = 0;
        radius = 2.5f; // Adjust as necessary for the vehicle size
        maxspeed = 40.0f; // Adjust maximum speed
    }
    ...
    void update()
    {
        // Update vehicle position, orientation, etc.
    }

    void applyDamage(float amount)
    {
        float effectiveDamage = amount - armor; // Armor reduces damage
        health -= effectiveDamage;
        if (health <= 0)
        {
            health = 0;
            destroyed = true;
            // Handle destruction effects, respawn logic, etc.
        }
    }
    ...
    {
        // Adjust vehicle orientation (yaw, pitch, roll)
    }

    void roll(float amount)
    {
        // Apply roll effect due to movement or damage
        maxroll = clamp(maxroll + amount, -10.0f, 10.0f);
    }
    ...
    void respawn()
    {
        ...
        acceleration = 5.0f;
        destroyed = false;
        // Reset position, orientation, etc.
    }
};
